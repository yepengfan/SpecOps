...
**Input**: Design documents from `/specs/001-sdd-cockpit/`
...
- [ ] T009 [P] Unit tests for section template constants in `__tests__/unit/sections.test.ts`: verify Requirements has 3 sections, Design has 5, Tasks has 4, all have id and title
...
- [ ] T012 Create section template constants in `lib/types/sections.ts`: fixed section definitions for Requirements (3 sections), Design (5 sections), Tasks (4 sections) per README
...
- [ ] T019 [US1] Create "New Project" dialog component in `components/ui/new-project-dialog.tsx`: text input for project name, validation (non-empty, max 100 chars, trimmed), submit creates project via `createProject()` and navigates to `/project/[id]/requirements`
- [ ] T020 [US1] Create project card component in `components/ui/project-card.tsx`: display project name, derived phase status (Requirements/Design/Tasks/Complete), last updated, click to navigate to active phase, delete button with confirmation dialog
...
**Goal**: Phases unlock sequentially (Requirements → Design → Tasks). Editing an approved phase resets downstream phases to draft with confirmation. All tested with manual content entry.

**Independent Test**: Manually type content into all requirements sections, approve, verify design unlocks. Try to navigate to tasks while design is draft — should be blocked. Approve design, edit a requirements section — confirm warning appears, downstream phases reset to draft.
...
- [ ] T026 [US4] Create phase navigation component in `components/phase/phase-nav.tsx`: tab bar with Requirements, Design, Tasks tabs. Show lock icon on locked phases, check icon on reviewed phases. Disable click on locked tabs. Use `role="tablist"` / `role="tab"` / `role="tabpanel"` for accessibility.
...
- [ ] T030 [US4] Create requirements phase page in `app/project/[id]/requirements/page.tsx`: display fixed sections in section editors, "Mark as Reviewed" button. No AI generation — manual content entry only.
- [ ] T031 [US4] Create design phase page in `app/project/[id]/design/page.tsx`: display fixed sections in section editors, "Mark as Reviewed" button, gate check redirects to active phase if design is locked
...
## Phase 6: User Story 3 - AI Requirements Generation (Priority: Critical)

**Goal**: Developers enter a natural language project description and the AI generates EARS-format requirements. "Generate" and "Regenerate" buttons added to the existing requirements page and section editors.
...
- [ ] T040 [US3] Create requirements system prompt in `lib/prompts/requirements.ts`: instruct Claude to generate EARS-format output with Problem Statement, EARS-format Requirements (WHEN/THEN/WHERE/IF), and Non-Functional Requirements sections
...
- [ ] T042 [US3] Add "Generate" button and project description input to requirements page in `app/project/[id]/requirements/page.tsx`: description textarea (min 10 chars), "Generate" button triggers streaming generation, populates section editors with generated content, loading state during generation
...
- [ ] T044 [US3] Handle malformed LLM responses in `app/project/[id]/requirements/page.tsx`: if response doesn't match expected section structure, display raw response with warning banner "Generated content may not follow the expected format", allow retry

**Checkpoint**: At this point, User Story 3 should be fully functional and testable independently

---

## Phase 7: User Story 5 - AI Design Generation (Priority: Critical)

**Goal**: After requirements are approved, AI generates a design.md draft with Architecture, API Contracts, Data Model, Tech Decisions, Security & Edge Cases sections.

**Independent Test**: Approve requirements phase, navigate to design, click "Generate", see design sections appear. Edit and approve.
...
- [ ] T046 [US5] Create design system prompt in `lib/prompts/design.ts`: instruct Claude to generate design document with Architecture, API Contracts, Data Model, Tech Decisions, Security & Edge Cases sections, using approved requirements as context
- [ ] T047 [US5] Add "Generate" button and AI generation to design page in `app/project/[id]/design/page.tsx`: "Generate" button sends approved requirements content as context, populates section editors, loading state, section regeneration. Reuse `stream-client.ts` from US3.
...
**Goal**: After design is approved, AI generates a tasks.md draft with Task List, Dependencies, File Mapping, Test Expectations sections.

**Independent Test**: Approve design phase, navigate to tasks, click "Generate", see task breakdown sections appear. Edit and approve.
...
- [ ] T049 [US6] Create tasks system prompt in `lib/prompts/tasks.ts`: instruct Claude to generate task breakdown with Task List (ordered, atomic), Dependencies, File Mapping, Test Expectations sections, using approved requirements AND design as context
- [ ] T050 [US6] Add "Generate" button and AI generation to tasks page in `app/project/[id]/tasks/page.tsx`: "Generate" button sends approved requirements + design content as context, populates section editors, loading state, section regeneration. Reuse `stream-client.ts` from US3.
...
**Independent Test**: Approve all three phases, click "Export", download zip, verify it contains `requirements.md`, `design.md`, `tasks.md` with correct content matching the latest saved content.

### Tests for User Story 7

> **NOTE: Write these tests FIRST, ensure they FAIL before implementation**

- [ ] T051 [P] [US7] Unit tests for markdown export in `__tests__/unit/export.test.ts`: verify each phase generates correct markdown with section titles and content in order, verify EARS format preserved in exported requirements
- [ ] T052 [P] [US7] Unit tests for zip export in `__tests__/unit/zip-export.test.ts`: verify zip contains three files (requirements.md, design.md, tasks.md), verify content matches latest saved content

### Implementation for User Story 7

- [ ] T053 [US7] Create markdown export utility in `lib/export/markdown.ts`: generate `requirements.md`, `design.md`, `tasks.md` from project sections using fixed section templates. Each file concatenates section titles and content in order.
...
- [ ] T068 [US8] Parse and persist AI-generated mappings in design generation flow (`app/project/[id]/design/page.tsx`): extract mapping metadata from LLM response, create `TraceabilityMapping` records with `origin: "ai"`, persist to project record in IndexedDB
...
- [ ] T071 [US8] Create traceability matrix table component in `components/traceability/matrix-table.tsx`: requirements as rows, design sections + tasks as columns, cell indicators (linked/gap), gap row highlighting (amber/red), coverage percentage display, AI vs manual mapping distinction (icon/label)
...
- **US3 - Requirements Gen (Phase 6)**: Depends on US4 (requirements page exists) + US2 (API key configured). Adds "Generate" to existing page.
...
7. Complete Phase 6: US3 - AI Requirements Generation (with tests)
8. **STOP and VALIDATE**: Generate requirements via AI, edit, approve, verify full workflow
